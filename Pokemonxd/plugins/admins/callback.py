...
from Pokemonxd.utilities.config import (AUTO_DOWNLOADS_CLEAR, BANNED_USERS,
                    SOUNCLOUD_IMG_URL, STREAM_IMG_URL,
                    TELEGRAM_AUDIO_URL, TELEGRAM_VIDEO_URL, adminlist)
from Pokemonxd import YouTube, bot
from Pokemonxd.modules.core.call import poke
from Pokemonxd.misc import SUDOERS, db
from Pokemonxd.modules.database import (is_active_chat,
...
from Pokemonxd.modules.utils.formatters import seconds_to_min
from Pokemonxd.utilities.inline.play import (panel_markup_1,
                                          panel_markup_2,
                                          panel_markup_3,
                                          stream_markup,
                                          telegram_markup)
...
@bot.on_callback_query(filters.regex("PanelMarkup") & ~BANNED_USERS)
@languageCB
...
@bot.on_callback_query(filters.regex("Pages") & ~BANNED_USERS)
@languageCB
async def del_back_playlist(client, CallbackQuery, _):
    await CallbackQuery.answer()
    callback_data = CallbackQuery.data.strip()
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await poke.unmute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await poke.stop_stream(chat_id)
                except:
                    return
        except:
            try:
